...
function initMap() {
    ...
    pauseBtn.addEventListener("click", () => {
        if (currentAudio && !currentAudio.paused) {
            ...
            currentAudio.pause();
            pauseBtn.textContent = "▶️"; // Change to 'Play' icon
        } else if (currentAudio && currentAudio.paused) {
            // We are paused, so RESUME
            currentAudio.play();
            pauseBtn.textContent = "⏸️"; // Change to 'Pause' icon
        }
    });
...
async function calculateAndDisplayRoute() {
    ...
    try {
        ...
        // 0.001 degrees ≈ 111 meters, so we scale based on distance
...
function checkDirection() {
    ...
    if (previousLocation && currentLocation) {
        ...
        if (isOnTrack) {
            directionMessage = "✓ On the right track!";
            directionClass = "on-track";
        } else if (angleDiff < 45) {
            directionMessage = "→ Heading in the right direction";
            directionClass = "on-track";
        } else if (angleDiff < 90) {
            directionMessage = "↗ Slightly off course";
            directionClass = "off-track";
        } else {
            directionMessage = "⚠ Going in wrong direction";
            directionClass = "off-track";
        }
    } else {
        if (isOnTrack) {
            directionMessage = "✓ On the route";
            directionClass = "on-track";
        } else {
            directionMessage = "📍 Move closer to the route";
            directionClass = "off-track";
        }
    }
    ...
    if (directionMessage && 
        ...
        if (directionMessage === "✓ On the right track!" && lastSpokenInstruction) {
